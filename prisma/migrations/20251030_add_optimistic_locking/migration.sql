...
-- Create new LockStatus enum (with idempotency check)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'LockStatus') THEN
    CREATE TYPE "LockStatus" AS ENUM ('UNLOCKED', 'LOCKED', 'EXPIRED');
  END IF;
END $$;

-- Enhance ChangeType enum (create new one and migrate, with idempotency check)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ChangeType_new') THEN
